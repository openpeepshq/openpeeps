...
const colorMap = {
  primary: { bg: 'bg-primary', text: 'text-primary', fg: 'text-primary-foreground' },
  secondary: { bg: 'bg-secondary', text: 'text-secondary', fg: 'text-secondary-foreground' },
  tertiary: { bg: 'bg-tertiary', text: 'text-tertiary', fg: 'text-tertiary-foreground' },
  success: { bg: 'bg-success', text: 'text-success', fg: 'text-success-foreground' },
  warning: { bg: 'bg-warning', text: 'text-warning', fg: 'text-warning-foreground' },
  error: { bg: 'bg-error', text: 'text-error', fg: 'text-error-foreground' },
  surface: { bg: 'bg-surface-100', text: 'text-foreground', fg: 'text-foreground' },
} as const;
...
export function variantClasses(variant?: Variant): string {
  ...
  switch (kind) {
    ...
    case 'ringed':
      return cn('border-2', `border-${color === 'surface' ? 'surface-400' : color}`, c.text);
    default:
      return '';
  }
}
