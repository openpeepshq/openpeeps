...
export const CompactReplyParent = ({
  ...
  return (
    <a
      ...
      className={cn(
        'relative flex max-h-20 items-center gap-2 overflow-hidden',
        className,
      )}
    >
      <div className="relative flex w-12 shrink-0 items-center self-stretch">
        <Avatar profile={post.profile} size={3} borderless />
        {showRail ? (
          <div className="bg-border-2 pointer-events-none absolute bottom-0 left-6 top-[calc(50%+1.5rem)] w-px" />
        ) : null}
      </div>
      {post.deletedAt ? (
