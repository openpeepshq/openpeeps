...
/**
 * Full-width image drop-zone mirroring the Svelte `ImageInput` (`displayType
 * "full"`): a dashed box that prompts for an upload when empty and shows the
 * image as a cover with replace/remove controls once set. Cropping happens in
 * `ImageEditModal`; the cropped file is uploaded and the resulting URL is
    ...
    >
      {url ? (
        <div className="relative z-10 flex items-center gap-4">
          <button
            type="button"
            title={t('form.imageInput.replace', { defaultValue: 'Replace image' })}
          ...
          <button
            type="button"
            title={t('form.imageInput.remove', { defaultValue: 'Remove image' })}
            className="bg-surface-200/80 flex size-10 items-center justify-center rounded-full"
        ...
        >
          ...
          <span className="font-bold">
            {text ?? t('form.imageInput.uploadText', { defaultValue: 'Upload Image' })}
          </span>
