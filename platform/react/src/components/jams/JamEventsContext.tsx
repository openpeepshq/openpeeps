...
import { parseJamEventPayload, sendJamEvent, sendReaction } from './jamEventActions';
...
export function JamEventsProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <JamEventsContext.Provider value={value}>{children}</JamEventsContext.Provider>
  );
}
...
export function useJamEventsContext(): JamEventsContextValue {
  ...
  if (!ctx) {
    throw new Error('useJamEventsContext must be used inside <JamEventsProvider>');
  }
  return ctx;
}
