...
export const isSsoAddKind = (value: string): value is SsoProviderKind =>
  (SSO_ADD_KINDS as readonly string[]).includes(value);

export const ssoKindDefaultId = (kind: SsoProviderKind): string => {
  switch (kind) {
    case 'gitlab-selfhosted':
      return 'gitlab';
    case 'entra':
...
export const uniqueSsoProviderId = (
  base: string,
