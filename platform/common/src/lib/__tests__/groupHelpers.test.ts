...
describe('groupHelpers', () => {
  ...
  describe('groupCapabilityTemplates', () => {
    it('should have all expected templates', () => {
      ...
      expect(groupCapabilityTemplates.announcementGroup).toBeDefined();
    ...
    it('includes local capabilities on member and moderator when local is defined', () => {
      for (const template of [
        groupCapabilityTemplates.defaultGroup,
        groupCapabilityTemplates.defaultGroupClosedCommunity,
        groupCapabilityTemplates.localGroup,
      ]) {
        const localAdds = template.capabilities.local?.add ?? [];
        const memberAdds = template.capabilities.member?.add ?? [];
        const moderatorAdds = template.capabilities.moderator?.add ?? [];
        for (const cap of localAdds) {
          expect(memberAdds, `${template.name} member`).toContain(cap);
          expect(moderatorAdds, `${template.name} moderator`).toContain(cap);
        }
      }
    });

    it('defaultGroup member includes join and react from local', () => {
      const memberAdds =
        groupCapabilityTemplates.defaultGroup.capabilities.member?.add ?? [];
      expect(memberAdds).toContain('core-groups-join');
      expect(memberAdds).toContain('core-posts-react');
    });
    ...
    it('should have correct structure for announcementGroup', () => {
      const template = groupCapabilityTemplates.announcementGroup;
      expect(template.name).toBe('announcementGroup');
      expect(template.capabilities.member).toBeDefined();
      expect(template.capabilities.moderator).toBeDefined();
      expect(template.capabilities.admin).toBeDefined();
      expect(template.capabilities.owner).toBeDefined();
