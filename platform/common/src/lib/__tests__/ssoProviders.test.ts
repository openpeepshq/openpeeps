...
describe('uniqueSsoProviderId', () => {
  it('uses the kind slug when it is free', () => {
    expect(ssoKindDefaultId('github')).toBe('github');
    expect(ssoKindDefaultId('gitlab-selfhosted')).toBe('gitlab');
    expect(ssoKindDefaultId('entra')).toBe('microsoft');
    expect(uniqueSsoProviderId('github', [])).toBe('github');
  });

  it('appends -2, -3, … when the slug is taken', () => {
    expect(uniqueSsoProviderId('github', ['github'])).toBe('github-2');
    expect(uniqueSsoProviderId('github', ['github', 'github-2'])).toBe(
      'github-3',
    );
  });
});
