...
const t: EventRecurrenceTranslate = (key, options) => {
  ...
  const dayMatch = key.match(/^events\.form\.repeat\.day\.([A-Z]{2})$/);
  if (dayMatch) {
    return dayLabels[dayMatch[1]] ?? key;
  }
...
describe('formatEventRecurrence', () => {
  it('formats weekly recurrence with weekdays and until', () => {
    expect(
      formatEventRecurrence(
        {
          freq: 'WEEKLY',
    ...
    ).toMatch(/^Weekly on Mon, Wed until /);
  });

  it('formats daily interval and count', () => {
    expect(
      formatEventRecurrence({ freq: 'DAILY', interval: 2, count: 5 }, t),
    ).toBe('Every 2 days, 5 times');
  });

  it('returns an empty string without recurrence', () => {
    expect(formatEventRecurrence(undefined, t)).toBe('');
  });
});
