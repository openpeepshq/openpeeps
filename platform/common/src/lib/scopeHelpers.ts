...
const calculateRequiredScopeLevel = (
  ...
  !!scopes.find(
    (s) =>
      (!s.scopeLevel ||
        ...
        includesScopeLevel(s.scopeLevel, requiredScope.scopeLevel)) &&
      (!s.resource.id ||
...
export const getPublicGroupReadScope = (groupId: string): Scope => ({
  scopeLevel: 'read',
  resource: { type: 'groups', id: groupId },
});

/**
 * Groups that grant the needed *-read caps to relationship `none` are readable
 * without auth (same pattern as {@link withPublicPostReadScopes}).
 */
export const withPublicGroupReadScopes = (
  authData: AuthorizationData,
  group: { id: string; capabilities?: { none?: { add?: string[] } } | null },
  neededCapabilities: string[],
): AuthorizationData => {
  if (!neededCapabilities.every((c) => c.endsWith('-read'))) {
    return authData;
  }
  const noneAdd = group.capabilities?.none?.add ?? [];
  if (!neededCapabilities.every((c) => noneAdd.includes(c))) {
    return authData;
  }
  const publicReadScope = getPublicGroupReadScope(group.id);
  const scopes = authData.scopes ?? [];
  if (scopeMatches({ scopes, requiredScope: publicReadScope })) {
    return authData;
  }
