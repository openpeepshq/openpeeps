...
export function PushEnabledDevices() {
  ...
  const remove = async (subscription: PushSubscription) => {
    ...
    try {
      ...
      if (isCurrentDevice(pushSubscriptionEndpoint(subscription) ?? undefined)) {
        await unsubscribePushNotifications();
      }
      setConfirmId(null);
      await subscriptionsQuery.refetch();
      setStatus(
        t('settings.notifications.pushEnabledDevices.delete.success', {
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {current ? (
                          <span className="bg-primary text-primary-foreground rounded-full px-2 py-0.5 text-[10px] font-bold uppercase">
                            {t('common.thisDevice', { defaultValue: 'This device' })}
                          </span>
                        ) : null}
                      </div>
