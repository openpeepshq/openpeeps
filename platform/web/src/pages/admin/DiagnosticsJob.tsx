...
function JobDetails({ job, onRefresh }: { job: AdminJobDetail; onRefresh: () => void }) {
  const t = useT();
    ...
    <div className="space-y-4 p-4">
      <div className="rounded-lg border bg-surface-100 p-4">
        ...
        <dl className="grid gap-2 text-sm sm:grid-cols-2">
          <div>
            <dt className="opacity-70">{t('diagnostics.jobs.queue', { defaultValue: 'Queue' })}</dt>
            <dd className="font-mono">{job.queue}</dd>
          </div>
          <div>
            <dt className="opacity-70">{t('diagnostics.jobs.id', { defaultValue: 'ID' })}</dt>
            <dd className="break-all font-mono">{job.id}</dd>
          </div>
          <div>
            <dt className="opacity-70">{t('diagnostics.jobs.name', { defaultValue: 'Name' })}</dt>
            <dd className="break-all font-mono">{job.name}</dd>
          </div>
          <div>
            <dt className="opacity-70">{t('diagnostics.jobs.state', { defaultValue: 'State' })}</dt>
            <dd className="font-mono">{job.state}</dd>
          </div>
          <div>
            <dt className="opacity-70">{t('diagnostics.jobs.queuedAt', { defaultValue: 'Queued at' })}</dt>
            <dd>{formatTime(job.timestamp)}</dd>
          </div>
          <div>
            <dt className="opacity-70">{t('diagnostics.jobs.finishedAt', { defaultValue: 'Finished at' })}</dt>
        ...
        {job.failedReason ? (
          <div className="border-error/30 bg-error/10 mt-3 rounded border p-2 text-sm">
            <p className="text-error mb-1 font-medium">
              {t('diagnostics.jobs.failedReason', { defaultValue: 'Failed reason' })}
            </p>
            <p className="whitespace-pre-wrap break-words">{job.failedReason}</p>
          </div>
        ) : null}
        <div className="mt-3">
          ...
          <pre className="bg-surface-200 max-h-64 overflow-auto rounded p-2 font-mono text-xs whitespace-pre-wrap break-words">
      ...
      <div className="rounded-lg border bg-surface-100 p-4">
        ...
        {job.logs.length === 0 ? (
          <p className="text-sm opacity-70">
            {t('diagnostics.jobs.logsEmpty', { defaultValue: 'No logs recorded.' })}
          </p>
        ) : (
          <ol className="space-y-1 font-mono text-xs">
            {job.logs.map((line, index) => (
              <li
                key={index}
                className="bg-surface-200 rounded px-2 py-1 whitespace-pre-wrap break-words"
              >
...
export function AdminDiagnosticsJob() {
  ...
  const refresh = () =>
    queryClient.invalidateQueries({
      queryKey: ['admin', 'diagnostics', 'jobs', queue, jobId],
    });
