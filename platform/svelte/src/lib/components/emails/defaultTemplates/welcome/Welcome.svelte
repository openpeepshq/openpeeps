...
<BaseEmailLayout 
	...
	showFooter={true}
>
	<Heading style={emailStyles.heading}>{t('emails.welcome.header')} {globals.communityConfig.info.name}!</Heading>
	
	<Text style={emailStyles.paragraph}>
		<OpenpeepsMarkdown class='text-base' source={globals.communityConfig.content?.welcomeEmail || 'Have fun!'} />
	</Text>
	
	<Text style={emailStyles.paragraph}>{t('emails.welcome.description')}:</Text>
		<Text style={emailStyles.paragraph}>
			<Link href={globals.serverData.rootUrl}>Visit {globals.communityConfig.info.name} now</Link>
		</Text>
		<Text style={emailStyles.paragraph}>
			<Link href="{globals.serverData.rootUrl}/settings">{t('emails.welcome.content.profile')}</Link> {t('emails.welcome.content.friends')}
		</Text>
		<Text style={emailStyles.paragraph}>
			<Link href="{globals.serverData.rootUrl}/settings/notifications"
				>{t('emails.welcome.content.notification')}</Link
			>
		</Text>
	
	<Section style={emailStyles.ctaContainer}>
		<Button href="{globals.serverData.rootUrl}" style={emailStyles.button}>{t('emails.welcome.cta')}</Button>
	</Section>

	{#if globals.communityConfig.info.contactEmail}
	<Text>
		{t('emails.welcome.content.contact')} <Link
			href="mailto:{globals.communityConfig.info.contactEmail}"
			>{globals.communityConfig.info.contactEmail}
		</Link> {t('emails.welcome.content.mail')}
	</Text>
	{/if}
</BaseEmailLayout>
