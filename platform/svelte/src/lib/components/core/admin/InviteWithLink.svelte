<script lang="ts">
	...
	import type { InviteLink } from '@openpeeps/common/types';
	...
	const handleCreateNewInviteLink = async () => {
		...
		inviteLink = await createInviteLink({
			maxUses: numberOfUses === t('admin.inviteLink.unlimited') ? 1000 : Number(numberOfUses),
	...
	<article class="flex flex-col p-3">
		...
		{#if inviteLink}
			<div class="bg-surface-50 mx-auto my-2 w-[96%] rounded-md border p-2">
				...
				<div class="text-center">{t('admin.inviteLink.totalLinkUses', { uses: 0 })}</div>
			</div>
		{/if}
		{#if inviteLink}
