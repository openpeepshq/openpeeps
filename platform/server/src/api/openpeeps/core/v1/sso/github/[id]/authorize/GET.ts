...
import { authorize } from '#lib/handlers/sso/oauth2';
...
export const apiEndpoint = endpoint({ Param, Error }).handle(
  async (input: { id: string }, event) => {
    ...
    try {
      const authUrl = await authorize('github', input.id, params);
      ...
      const errorUrl = new URL(`/auth/sso/github/${input.id}/callback`, origin);
