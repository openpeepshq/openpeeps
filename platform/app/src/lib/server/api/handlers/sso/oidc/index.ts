...
import type { CoreConfig, ProfileData, TokenResponse, ProfileWithMeta } from '@openpeeps/common/types';
...
const extractProfileDataFromOidc = async (
  ...
  config: CoreConfig['sso']['oidc'][number],
  email: string,
): Promise<ProfileData> => {
  const handleKey = config.claimMapping?.handle || 'preferred_username';
  ...
  const avatar = config.claimMapping?.avatar
    ? (claims[config.claimMapping.avatar] as string | undefined)
    : undefined;

  const displayNameKey = config.claimMapping?.displayName || 'name';
...
const fetchToken = async (
  ...
  if (!tokenResponse.ok) {
    ...
    log.error({ status: tokenResponse.status, body: errorBody }, 'OIDC token exchange failed');
...
export const authorize = async (
  ...
  if (!provider) {
    const available = appConfig.sso.oidc.map(p => p.id).join(', ') || '(none configured)';
    throw new Error(`OIDC provider "${providerId}" not found. Available: ${available}`);
  }
  ...
  await (await getSharedConnection()).setEx(
...
export const callback = async (
...
): Promise<{ success: boolean; token: string; error?: string; redirectState?: string; redirectUrl?: string }> => {
  const { code, state } = queryParams;
  log.debug({ code, state: state?.substring(0, 50) }, 'OIDC callback params');

  if (queryParams.error) {
    const desc = queryParams.error_description || '';
    throw new Error(`OIDC authorization error: ${queryParams.error}${desc ? `: ${desc}` : ''}`);
  }

  if (!code || !state) {
    throw new Error(`Missing authorization code or state parameter. code=${!!code}, state=${!!state}`);
  }
  ...
  if (!storedData) {
    log.error({ stateId: stateId.substring(0, 40) }, 'PKCE state not found in Redis (expired or already used)');
  ...
  if (!claims || Object.keys(claims).length === 0) {
    claims = accessToken
      ? await fetchUserinfo(provider, accessToken)
      : {};
  }

  log.info({ claims: JSON.stringify(claims), accessToken: accessToken?.substring(0, 20) }, 'OIDC resolved claims');
  ...
  if (!email || !z.string().email().safeParse(email).success) {
    const username = (claims.preferred_username || claims.username || claims.nickname || claims.sub) as string | undefined;
    const issuer = claims.iss as string | undefined;
    if (username && issuer) {
      try {
        const issuerDomain = new URL(issuer).hostname;
        email = `${username}@${issuerDomain}`;
        log.info({ email }, 'OIDC: constructed fallback email from username + issuer domain');
      } catch {
  ...
  if (existingAccount) {
    let profile: ProfileWithMeta | undefined = (await listProfilesByAccount(existingAccount))[0];
  ...
  if (approvalRequired) {
    const redirectState = Buffer.from(JSON.stringify({ ...pkceState, pendingEmail: normalizedEmail })).toString('base64');
    return {
      success: false,
      token: '',
