<script lang="ts">
  ...
  import { ConfigCommunityLinks } from '@openpeeps/svelte/components';
  ...
  pageHeaderStore.set({
    title: t('configuration.community.links.title'),
  });
</script>
...
<div class="p-4">
  <Loader queries={[$configQuery]}>
    {#if $configQuery.data}
      <ConfigCommunityLinks defaults={$configQuery.data.config} />
    {/if}
  </Loader>
</div>
