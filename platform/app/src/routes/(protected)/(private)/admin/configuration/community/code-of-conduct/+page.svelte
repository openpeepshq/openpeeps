<script lang="ts">
  ...
  import { ConfigCommunityCodeOfConductPage } from '@openpeeps/svelte/components';
  ...
  pageHeaderStore.set({
    title: t('configuration.community.codeOfConduct.title'),
  });
</script>
...
<div class="p-4">
  <Loader queries={[$configQuery]}>
    {#if $configQuery.data}
      <ConfigCommunityCodeOfConductPage defaults={$configQuery.data.config} />
    {/if}
  </Loader>
</div>
