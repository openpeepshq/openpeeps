<script lang="ts">
  ...
  import { ConfigCommunityInfo } from '@openpeeps/svelte/components';
  ...
  pageHeaderStore.set({
    title: t('configuration.community.info.title'),
  });
</script>
...
<div class="p-4">
  <Loader queries={[$configQuery]}>
    {#if $configQuery.data}
      <ConfigCommunityInfo defaults={$configQuery.data.config} />
    {/if}
  </Loader>
</div>
