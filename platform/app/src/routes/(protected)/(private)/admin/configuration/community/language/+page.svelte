<script lang="ts">
  ...
  import { ConfigCommunityLanguage } from '@openpeeps/svelte/components';
  ...
  pageHeaderStore.set({
    title: t('configuration.community.language.title'),
  });
</script>
...
<div class="p-4">
  <Loader queries={[$configQuery]}>
    {#if $configQuery.data}
      <ConfigCommunityLanguage defaults={$configQuery.data.config} />
    {/if}
  </Loader>
</div>
