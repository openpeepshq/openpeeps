<script lang="ts">
  ...
  import { ConfigCommunityAboutPage } from '@openpeeps/svelte/components';
  ...
  pageHeaderStore.set({
    title: t('configuration.community.aboutPage.title'),
  });
</script>
...
<div class="p-4">
  <Loader queries={[$configQuery]}>
    {#if $configQuery.data}
      <ConfigCommunityAboutPage defaults={$configQuery.data.config} />
    {/if}
  </Loader>
</div>
