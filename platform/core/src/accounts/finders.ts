...
export const findAccount = (id: string) => getAccount(id);
...
export const existsAccountByEmail = (email: string) =>
  allpeepDb()
    .then(({ db }) =>
      accountsMapping.ignoreSoftDelete().findOneBy(db, { matches: { email } }),
    )
    .then(Boolean);

export const findByProfile = (profile: Profile) =>
  allpeepDb().then(({ db }) =>
    profilesMapping.relationsFrom(profile, profileAccountRelation).all(db),
  );
