...
import { geocoder, type GeocodingResult, type GeocoderType } from "@openpeeps/geocoder";
...
const getNominatimResults = async (query: string): Promise<GeocodingResult[]> =>
    cache.wrap(query, async () => {
        ...
        const remoteGeocoder = geocoder((coreConfig.services.location.type || 'nominatim') as GeocoderType, {
