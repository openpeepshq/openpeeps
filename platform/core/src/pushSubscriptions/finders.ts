...
export const findPushSubscription = (id: string) => allpeepDb().then(db => pushSubscriptionsMapping.find(db.db, id));

export const listPushSubscriptionsByAccount = (account: Account) =>
    allpeepDb().then(({ db }) => accountsMapping.relationsFrom(account, pushSubscriptionsRelation).all(db));