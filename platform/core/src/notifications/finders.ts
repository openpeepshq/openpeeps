...
export const listNotificationsByProfile = (
  profile: ProfileWithMeta,
  { start, limit = 100 }: { start?: string; limit?: number } = { limit: 100 },
) =>
  allpeepDb().then(({ db }) =>
    filterAndTransform<DbNotification, ExpandedNotification>(
      addStart<DbNotification>(baseListNotifications(profile), start),
      db,
      { transform: expandNotification, limit },
    ),
  );
