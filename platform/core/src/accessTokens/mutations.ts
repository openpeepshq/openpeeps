import { AccessTokenData, AccountWithMeta, ProfileWithMeta } from "@openpeeps/common/types";
import { allpeepDb } from "../db";
import { profileAccessTokenConnector, removeEdgesForAccessToken } from "./helpers";
import { accessTokensMapping } from "./mapping";
import { uuidv7 } from "uuidv7";
import { addToBloomFilter } from "./revokations";

export const createAccessToken = (profile: ProfileWithMeta | undefined, data: AccessTokenData) =>
...
export const revokeAccessToken = (id: string) =>
    allpeepDb().then(async ({ db }) => {
        const accessToken = await accessTokensMapping.update(db, id, { revokedAt: new Date().toISOString() });
        addToBloomFilter(id);
        return accessToken;
    });
