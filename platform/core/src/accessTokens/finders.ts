...
const withCachedOwnerAndScopes = async (
  token: AccessToken,
): Promise<AccessTokenWithScopes> => {
...
export const findAccessToken = (
  id: string,
): Promise<AccessTokenWithScopes | undefined> =>
  allpeepDb().then(({ db }) =>
    accessTokensMapping
      .find(db, id)
      .then((t) => (t ? withCachedOwnerAndScopes(t) : undefined)),
  );
...
export const listAccessTokensForProfile = (
  profile: ProfileWithMeta,
): Promise<AccessTokenWithScopes[]> =>
  allpeepDb().then(async ({ db }) =>
    profilesMapping
      .relationsFrom(profile, profileAccessTokensRelation)
      .all(db)
