...
export const verifySignedAccessToken = (token: string): Promise<Authorization | undefined> =>
    jwtUtil().then((jwt) => jwt.verify(token)).then(async (result) => {
...
export const createSignedAccessToken = async ({
    ...
    const signedToken = await jwt.sign({ payload: authorization, expirationTime, id });
    ...
    return createAccessToken(profile, { name, description, signedToken, expiresAt });
}
...
export const createSignedProfileAccessToken = async ({
    ...
    return createSignedAccessToken({ profile, authorization, name, description, expirationTime });
}


export const createSignedServiceToken = async ({
    scopes, name, description, expirationTime = '1y'
}: {
    scopes: Scope[],
    name: string,
    description?: string,
 ...
 */
export const refreshSignedAccessToken = async (token: string): Promise<AccessToken | undefined> => {
    const jwt = await jwtUtil();
    ...
    if (existing.expiresAt && existing.createdAt) {
        ttlSec = Math.floor(
            (new Date(existing.expiresAt).getTime() - new Date(existing.createdAt).getTime()) / 1000,
        );
