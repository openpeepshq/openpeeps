...
export const addToBloomFilter = (id: string) =>
    getBloomFilterConnection().then((redis) => redis.bf.add(revocationsBloomFilterKey, id));

export const isMaybeRevoked = (id: string) =>
    getBloomFilterConnection().then((redis) => redis.bf.exists(revocationsBloomFilterKey, id));
