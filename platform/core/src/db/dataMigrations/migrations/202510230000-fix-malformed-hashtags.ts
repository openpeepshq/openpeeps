...
export default {
    ...
    migration: async (db: Database) => {
        ...
        if (!(await hashtagsCollection.exists()) || !(await postHashtagsCollection.exists())) {
            log.info('Required collections do not exist, skipping migration');
            return;
        }

        // Get all hashtags and filter malformed ones in JavaScript
        const allHashtags = await db.query(aql`FOR h IN hashtags RETURN h`).then(r => r.all());
        const malformedHashtags = allHashtags.filter((h: any) => h.tag && /[^a-zA-Z0-9_-]/.test(h.tag));
        ...
        for (const malformedHashtag of malformedHashtags) {
            try {
                ...
                const existingHashtag = await db.query(aql`
                    FOR h IN hashtags 
                ...
                if (existingHashtag) {
                    ...
                    log.info(`Merging malformed hashtag "${malformedHashtag.tag}" with existing clean hashtag "${validTag}"`);
                    
                    // Move all connections from malformed to existing clean hashtag
                    const connections = await db.query(aql`
                    ...
                    for (const edge of connections) {
                        ...
                        const existingConnection = await db.query(aql`
                    ...
                    log.info(`Fixing hashtag in place: "${malformedHashtag.tag}" -> "${validTag}"`);
