...
export const createReport = async (
  ...
  await Promise.all(
    reportedPosts.map((post) => reportedPostsConnector(db, report, post)),
  );
...
export const resolveReport = async (
  ...
  allpeepDb().then(({ db }) =>
    reportsMapping.update(db, report.id, { resolution }),
  );

export const reopenReport = async (report: ReportWithMeta) =>
  allpeepDb().then(({ db }) =>
    reportsMapping.update(db, report.id, { resolution: undefined }),
  );
