...
): Promise<ReportWithMeta[]> => Promise.all(reports.map(transformReport));

export const createdReportConnector = connector<
  ProfileWithMeta,
  ReportWithMeta
>(
...
export const reportedProfileConnector = connector<
  ReportWithMeta,
  ProfileWithMeta
>(
  collectionInfos.reportsCollection,
