import { AdminServerStats, PublicPostWithActivityScore, ObjectStatsWithAll, PublicProfileWithActivityScore, publicProfileWithActivityScoreSchema, publicPostWithActivityScoreSchema } from "@openpeeps/common/types";
import { sub } from "date-fns";
import { Database } from "arangojs";
import { baseProfilesMapping } from "../profiles";
import { allpeepDb } from "../db";
import { postsMapping, postsWithReplyCountMapping, postsWithReplyToCountMapping, transformPost } from "../posts";
import { entriesMapping, followsMapping, postsWithActivityScoreMapping, profileWithActivityScoreMapping, reactionsMapping, repostsMapping } from "./mapping";
import { compileStatsWithAll, creationDateFilter } from "./helpers";
import { jamParticipantsMapping, jamSessionStartsMapping } from "../jams/mapping";

export const activeProfilesCount = (db: Database, start?: Date, end?: Date) =>
    profileWithActivityScoreMapping(start, end).filter('DOC.activityScore > 0').count(db);
export const activeProfilesStats = (): Promise<ObjectStatsWithAll> => compileStatsWithAll(activeProfilesCount);


export const profilesCount = (db: Database, start?: Date, end?: Date) => baseProfilesMapping.filter(creationDateFilter(start, end)).count(db);
export const profileStats = (): Promise<ObjectStatsWithAll> => compileStatsWithAll(profilesCount);

export const postsCount = (db: Database, start?: Date, end?: Date) => postsMapping.filter(creationDateFilter(start, end)).count(db);
export const postsStats = (): Promise<ObjectStatsWithAll> => compileStatsWithAll(postsCount);

export const postsWithoutReplyCount = (db: Database, start?: Date, end?: Date) =>
    postsWithReplyCountMapping.filter(creationDateFilter(start, end)).filter({ matches: { replyCount: 0 } }).count(db);
export const postsWithoutReplyStats = (): Promise<ObjectStatsWithAll> => compileStatsWithAll(postsWithoutReplyCount);

export const postsWithRepliesCount = (db: Database, start?: Date, end?: Date) =>
    postsWithReplyCountMapping.filter(creationDateFilter(start, end)).filter('DOC.replyCount > 0').count(db);
export const postsWithRepliesStats = (): Promise<ObjectStatsWithAll> => compileStatsWithAll(postsWithRepliesCount);

export const postsRepliesCount = (db: Database, start?: Date, end?: Date) =>
    postsWithReplyToCountMapping.filter(creationDateFilter(start, end)).filter({ matches: { replyToCount: 1 } }).count(db);
export const postsRepliesStats = (): Promise<ObjectStatsWithAll> => compileStatsWithAll(postsRepliesCount);
...
export const interactionsStats = (): Promise<ObjectStatsWithAll> => compileStatsWithAll(interactionsCount);
...
export const jamSessionsStats = (): Promise<ObjectStatsWithAll> => compileStatsWithAll(jamSessionStarts);

const jamParticipants = (db: Database, start?: Date, end?: Date) =>
    jamParticipantsMapping.filter(creationDateFilter(start, end)).count(db);
export const jamParticipantsStats = (): Promise<ObjectStatsWithAll> => compileStatsWithAll(jamParticipants);

const topPosts = (db: Database, start: Date): Promise<PublicPostWithActivityScore[]> =>
...
const topProfiles = (db: Database, start: Date): Promise<PublicProfileWithActivityScore[]> =>
    profileWithActivityScoreMapping(start, new Date())
        ...
        .then(profiles => profiles
            .map(profile => publicProfileWithActivityScoreSchema.safeParse(profile))
            .filter(result => result.success)
            .map(result => result.data)
        );

export const topListsStats = () => allpeepDb().then(async ({ db }) => ({
    posts: {
...
export const newProfilesForInterval = (start: Date, end: Date) =>
    allpeepDb().then(({ db }) => baseProfilesMapping.filter(creationDateFilter(start, end)).count(db));
...
export const signupsTimeline = (duration: 'P1W' | 'P4W' | 'P120D') =>
    Promise.all(
        createIntervals(duration).map(
            interval => newProfilesForInterval(interval.start, interval.end)
                .then(count => ({
                    name: interval?.name,
                    count,
                }))
