...
const loadStripeSubscription = async (customerId: string): Promise<StripeSubscriptionData> => {
  const subscriptions = await getStripe().then(stripe => stripe.subscriptions.list({
    customer: customerId,
    limit: 1,
    status: 'all',
...
export const getUserStripeSubscription = async (customerId: string) =>
  stripeCache.wrap(customerId, async () =>
    loadStripeSubscription(customerId)
  );
