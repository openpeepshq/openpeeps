...
export const findRoleByKey = (key: string): Promise<Role | undefined> =>
  allpeepDb().then(({ db }) =>
    rolesMapping.findOneBy(db, { matches: { key } }),
  );

export const listRoles = (): Promise<Role[]> =>
  allpeepDb().then(({ db }) => rolesMapping.all(db));

export const findRolesByCapabilities = (
  capabilities: string[],
): Promise<Role[]> =>
  allpeepDb()
    .then(({ db }) => rolesMapping.all(db))
    .then((roles) =>
      roles.filter(
        (role) => checkRoleCapabilities([role], capabilities).success,
      ),
    );
