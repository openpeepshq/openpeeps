import type { GroupData, GroupRelationship, GroupWithMeta, PostWithMeta, ProfileWithMeta, PublicProfile } from "@openpeeps/common/types";
...
import { RelationWithMapping } from "@openpeeps/arango-querybuilder";
import { profilesCache } from "../profiles/cache";

export const addMembersToGroup = (group: GroupWithMeta, members: PublicProfile[]) =>
    allpeepDb().then(({ db }) => Promise.all(members.map((p) =>
        addMember(db, p, group, { roles: ['member'] }).then(() => profilesCache.del(p.id))
    )));

export const removeMembersFromGroup = (group: GroupWithMeta, members: PublicProfile[]) =>
    allpeepDb().then(({ db }) => Promise.all(members.map((p) =>
...
export const createGroup = async (
    ...
    const uniqueMembers = filteredMembers ? Array.from(new Set(filteredMembers.map(m => m.id))).map(id => filteredMembers.find(m => m.id === id)!) : [];
...
export const updateGroup = (group: GroupWithMeta, groupData: GroupData) =>
    allpeepDb().then(({ db }) =>
        groupsMapping.update(db, group.id, groupData),
    );

export const deleteGroup = async (group: GroupWithMeta) => {
    const { db } = await allpeepDb();
    await groupsMapping.delete(db, group.id);
    const posts = await groupsMapping.relationsFrom(group, {
        alias: 'posts',
        ...
        skipEdge: true,
    } as RelationWithMapping<PostWithMeta>).all(db);
